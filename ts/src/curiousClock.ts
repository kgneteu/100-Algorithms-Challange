...
// The seller never told Ben what was so special about it,
// but mentioned that one day Benjamin would be faced with a surprise.
//
// Indeed, the clock did surprise Benjamin: without warning,
// at someTime the clock suddenly started going in the opposite direction!
// Unfortunately, Benjamin has an important meeting very soon,
// and knows that at leavingTime he should leave the house so as to not be late.
// Ben spent all his money on the clock, so has to figure out what time his
// clock will show when it's time to leave.
//
...
export function curiousClock(someTime: string, leavingTime: string): string {
    const st = Date.parse(someTime)
    const lt = Date.parse(leavingTime)
    console.log(st)
    const ret = lt - st;
    const r = new Date()
        r.setTime(st - ret)
    console.log(r.toUTCString())
        return r.toISOString().split(/[T\.]/).slice(0, 2).join(" ").slice(0, -3)
}

console.log(curiousClock("2016-08-26 22:40",
    "2016-08-29 10:00"))
