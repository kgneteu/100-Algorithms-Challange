// All four trainers have solved the task correctly, so the
// answer is (3 + 6 + 4 + 5) / 4 = 4.5.
...
// is identified as trainers before the bot goes live,
// and they CodeFight against the bot during a training phase.
// The current training algorithm is definitely more complex,
...
// -1 if the answer was wrong, and 0 if no answer was given.
// In this case, the bot's correct answer time for a given task would be the average
// of the answer times from the trainers who answered correctly.
// Given all of the training information for a specific task, calculate the bot's answer time.
//
// Example
//
// For
// trainingData = [ [3, 1], [6, 1], [4, 1], [5, 1]] the output should be
// companyBotStrategy(trainingData) = 4.5.
//
// For
// trainingData = [ [4, 1], [4, -1], [0, 0], [6, 1]] the output should be
...
// For
//
// trainingData = [ [4, -1], [0, 0], [5, -1]] the output should be
...
//  [input] array.array.integer trainingData The ith element of trainingData contains exactly 2 elements:
//  trainingData[i][0] is the answer time for the ith person and trainingData[i][1] is correctness.
//  If a trainer answered correctly, then correctness equals 1; If a trainer answered incorrectly,
//  then correctness equals -1; If a trainer didn't give any answer, answerTime and correctness both equal to 0.
//
...
export function companyBotStrategy(trainingData: number[][]): number {
    ...
    return correctAnswers > 0 ? totalTime / correctAnswers : 0.0;
}
