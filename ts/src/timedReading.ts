...
// improve and advance reading skills.
// A young elementary student has just finished his very first timed
// reading exercise.
// Unfortunately he's not a very good reader yet,
// so whenever he encountered a word longer than maxLength,
// he simply skipped it and read on.
//
// Help the teacher figure out how many words the boy has read by
// calculating the number of words in the text he has read,
...
export function timedReading(maxLength: number, text: string): number {
    ...
    if (clean.trim().length === 0) return 0;
    const words = clean.split(/\s/).filter(v=>v.length<=maxLength)
    return words.length;
